...
public class ResideMenu extends FrameLayout{
    ...
    private void setScaleDirection(int direction){
        ...
        int ancho = (int) (screenWidth * 0.71f);

        if (getResources().getConfiguration().orientation == Configuration.ORIENTATION_LANDSCAPE)
            ancho = (int) (getScreenHeight() * 1.2f);

        if (direction == DIRECTION_LEFT){
            FrameLayout.LayoutParams params = (FrameLayout.LayoutParams) scrollViewLeftMenu.getLayoutParams();
            params.width = ancho;
            scrollViewLeftMenu.setLayoutParams(params);
            scrollViewMenu = scrollViewLeftMenu;
            scrollViewMenu.setLayoutParams(params);
            pivotX  = screenWidth * 1.5f;
        }else{
            FrameLayout.LayoutParams params = (FrameLayout.LayoutParams) scrollViewRightMenu.getLayoutParams();
            params.width = ancho;
            scrollViewRightMenu.setLayoutParams(params);
